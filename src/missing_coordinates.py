...
def missing_coordinates(df: pd.DataFrame, BATCH_SIZE: int):
    ...
    if len(df_missing_coordenates_copy) > 1:
        df_missing_coordenates_copy["searched_postcode"] = df_missing_coordenates_copy["postcode"].str.slice(-5)
        df_missing_coordenates_copy.apply(update_searched_postcode, axis=1)
    else:
        df_missing_coordenates_copy["searched_postcode"] = 37073   # city Göttingen was searched in the documentation
        df_missing_coordenates_copy.apply(update_searched_postcode, axis=1)
